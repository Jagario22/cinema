...
function handleTitleChange(e) {
    ...
    const freeTicketsOfTime = sessions[sessionId - 1].freeTicketsOfTime;
    let timeList = document.createElement("ul");
    timeList.className = "time";
    freeTicketsOfTime.forEach(function (timeElement, index) {
        let timeLi = document.createElement("li");
        const timeLink = document.createElement("a");
        Object.keys(timeElement)
            .forEach(function eachKey(key) {
                const time = key
                let places = timeElement[key];
                timeLink.textContent = time + ", " + placesTitle +": " + places;
...
function fillDropDown(data) {
    ...
    data.forEach(function (item, index) {
        ...
        const sessionDate = new Date(item.date.year, item.date.month-1, item.date.day)
