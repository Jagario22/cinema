...
function handleTitleChange(e) {
    ...
    const sessionsPlacesInfo = session.sessionsPlacesInfo;
    let timeList = document.createElement("ul");
    timeList.className = "time";
    sessionsPlacesInfo.forEach(function (sessionPlacesInfo, index) {
        let timeLi = document.createElement("li");
        const timeLink = document.createElement("a");
        const time = getTime(sessionPlacesInfo.sessionTime)

        timeLink.textContent = time + ", " + placesTitle + ": " + sessionPlacesInfo.freePlacesCount;
        timeLink.id = "time" + (index + 1);
        timeLink.href = getTicketUrl(sessionPlacesInfo.sessionId);
...
function fillPlaceSelect(data, selectedRow) {
    ...
    if (data != null) {
        ...
        data = data.filter(d => d.placeNumber <= countOfPlaces && d.placeNumber >= (countOfPlaces - options.length));
        for (let i = 0; i < data.length; i++) {
            let query = "\"" + data[i].placeNumber + "\"";
