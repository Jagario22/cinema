package com.epam.finalproject.cinema.web.constants;

public class CommandNames {
    public final static String WELCOME_PAGE_COMMAND = "welcome";
    public final static String LOGIN = "login";
    public final static String REGISTER = "register";
    public final static String SHOW_FILM_SESSIONS = "showSessions";
    public final static String SHOW_AVAILABLE_TICKETS = "showTickets";
    public final static String SHOW_MOVIE_INFO = "showMovie";
    public final static String SHOW_FiLM_SESSION_INFO = "showSession";
    public final static String SHOW_PROFILE = "showProfile";
    public final static String BUY_TICKET = "buyTicket";
    public static final String LOGOUT = "logout";
    public static final String TOP_UP_BALANCE = "topUpBalance";
    public static final String SHOW_ADD_MOVIE_PAGE = "showAddMovie";
    public static final String ADD_MOVIE = "addMovie";
    public static final String ADMIN_ADD_SESSION = "addSession";
    public static final String ADMIN_CANCEL_SESSION = "cancelSession";
    public static final String ADMIN_SHOW_SCHEDULE = "adminShowSchedule";
    public static final String ADMIN_SHOW_ALL_MOVIES_INFO = "showAllMoviesInfo";
    public static final String ADMIN_SHOW_ALL_USERS = "adminShowUsers";
    public static final String ADMIN_CHANGE_ROLE = "changeRole";
}
